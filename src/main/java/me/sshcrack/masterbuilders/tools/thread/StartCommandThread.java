...
public class StartCommandThread extends ThreadCommand {
    ...
    public CommandResponse onCommand(CommandSender sender, String label, String[] args) {
        ...
        String teamId = playerTeam.getName().toLowerCase();
        ...
        if (!hasPlot) {
            ...
            int offset = config.getInt("offset", 1);
            config.set("offset", offset +1);

            int spacing = 50;
            int blocksAway = maxSize * offset + spacing;
        ...
        Location startLoc = LocTools.lowestToConfigStart(minLoc, diffLoc);
        Location tpLoc = startLoc.clone().add(spawn);
        tpLoc.setYaw(spawn.getYaw());
        tpLoc.setPitch(spawn.getPitch());

        Bukkit.getLogger().info(String.format("MinLoc: x: %s, y: %s, z: %s", minLoc.getX(), minLoc.getY(), minLoc.getZ()));
        Bukkit.getLogger().info(String.format("Diff: x: %s, y: %s, z: %s", diffLoc.getX(), diffLoc.getY(), diffLoc.getZ()));
        Bukkit.getLogger().info(String.format("StartLoc: x: %s, y: %s, z: %s", startLoc.getX(), startLoc.getY(), startLoc.getZ()));
